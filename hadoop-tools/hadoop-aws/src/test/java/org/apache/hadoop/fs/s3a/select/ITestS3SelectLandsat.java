 ...
 *
 * Executed iff the destination store declares its support for the feature
 * and the test CSV file configuration option points to the standard
 * landsat GZip file
 *
 * Normally working with the landsat file is a scale test.
 * Here, because of the select operations, there's a lot less data
 * to download.
 * For this to work: write aggressive select calls.
 *
...
public class ITestS3SelectLandsat extends AbstractS3SelectTest {
  ...
  private FileSystem landsatFS;

  public static final int SELECT_LIMIT = 25;

  public static final String LIMIT = " LIMIT " + SELECT_LIMIT;

  public static final String SELECT_ALL_CLOUDS = "SELECT\n"
      + "s.entityId from\n"
      + "S3OBJECT s WHERE\n"
      + "s.\"cloudCover\" = '100.0'\n"
      + LIMIT;
  ...
  public void setup() throws Exception {
    ...
    landsatFS = landsatGZ.getFileSystem(conf);
    selectConf = new Configuration(false);
    // file is compressed.
    selectConf.set(s3a(INPUT_COMPRESSION), COMPRESSION_OPT_GZIP);
    // and has a header
  ...
  public void testSelectCloudcoverIgnoreHeader() throws Throwable {
    ...
    selectConf.set(s3a(CSV_INPUT_HEADER), HEADER_OPT_IGNORE);
    String sql = "SELECT\n"
        + "* from\n"
        + "S3OBJECT s WHERE\n"
        + "s._3 = '0.0'\n"
  ...
  public void testSelectCloudcoverUseHeader() throws Throwable {
    describe("select 100% cover using the header, verify projection too");
    selectConf.set(s3a(CSV_INPUT_HEADER), HEADER_OPT_USE);
    List<String> list = selectLandsatFile(selectConf, SELECT_ALL_CLOUDS);
    LOG.info("Line count: {}", list.size());
    verifySelectionCount(1, SELECT_LIMIT, SELECT_ALL_CLOUDS, list);
    String line1 = list.get(0);
    assertThat("no column filtering from " + SELECT_ALL_CLOUDS,
  ...
  private List<String> selectLandsatFile(
    ...
    return parseToLines(
        select(landsatFS, landsatGZ, conf, sql, args),
        SELECT_LIMIT * 2);
  }
