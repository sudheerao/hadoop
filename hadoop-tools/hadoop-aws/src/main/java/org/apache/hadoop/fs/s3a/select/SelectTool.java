...
public class SelectTool extends S3GuardTool {
  ...
  public int run(String[] args, PrintStream out)
    ...
    final List<String> parsedArgs = parseArgs(args);
    ...
    final CommandFormat command = getCommandFormat();
    final String header = command.getOptValue(OPT_HEADER);
    if (isNotEmpty(header)) {
      println(out, "Using header option %s", header);
      initialConf.set(CSV_INPUT_HEADER, header, "cli");
    }
    final String output = command.getOptValue(OPT_OUTPUT);
    Path destPath = null;
  ...
  public static double bandwidthMBs(long bytes, long durationNS) {
    return durationNS > 0
        ? (bytes / 1048576.0 * 1.0e9 / durationNS)
        : 0;
  }
}
